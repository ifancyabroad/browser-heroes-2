...
Ghosts use the same combat rules, equipment, skills, feats, and class tactics as other combatants. Player ghosts retain the cumulative zone level in which they died as well as their original hero level and build. Ghosts from battles 91–100 remain in the zone-ten pool, while all ghosts created after battle 100 share the endless pool and can appear throughout endless progression. Defeating a player ghost banishes it from ghost pools frozen after that victory. Older runs retain the player ghost in their frozen pool, preserving deterministic encounters, but a run never encounters a ghost it has already defeated. Ghost appearance and identity are derived from the run seed and battle context using only ghosts published and not yet banished at the run's ghost-pool cutoff. Normal runs take this cutoff when created; participants in the same Daily Challenge share it.

Before endless progression, when a frozen pool has no eligible player ghost for the zone, an authored system ghost appears instead. The endless pool has no system ghosts, so an empty endless player pool leaves the scheduled battle as a normal encounter. System ghosts cannot be globally banished, but a run does not encounter the same system ghost again after defeating it.
